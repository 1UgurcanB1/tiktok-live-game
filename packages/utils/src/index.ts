...
) => new Intl.DateTimeFormat("th-TH", { timeZone, ...opts })
    .format(new Date(input));

export const elapsed = (from: Date | string | number, to: Date | string | number = Date.now()) =>
...
export const uniqBy = <T>(arr: T[], key: (x: T) => string | number) => {
    ...
    for (const it of arr) { const k = key(it); if (!m.has(k)) m.set(k, it); }
    return [...m.values()];
};

export const groupBy = <T, K extends string | number | symbol>(
    arr: T[], key: (x: T) => K
): Record<K, T[]> => arr.reduce((acc, it) => {
    const k = key(it); (acc[k] ||= []).push(it); return acc;
}, {} as Record<K, T[]>);
...
export async function withTimeout<T>(p: Promise<T>, ms: number, msg = "Timeout") {
    let t: any; const timeout = new Promise<never>((_, rej) => t = setTimeout(() => rej(new Error(msg)), ms));
...
export async function retry<T>(
    ...
    for (let i = 1; i <= attempts; i++) {
        try { return await fn(); }
        catch (e) { last = e; onError?.(e, i); if (i < attempts) await sleep(delayMs * i); }
    }
    throw last;
}
